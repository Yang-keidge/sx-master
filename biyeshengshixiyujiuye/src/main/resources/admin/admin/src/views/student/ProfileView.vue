  ...
  <main class="profile-page">
    ...
    <section class="panel-card profile-panel">
      <aside class="profile-summary">
        ...
        <small>{{ account.yuanxiValue || '未设置院系' }} · {{ account.banjiValue || '未设置班级' }}</small>
      </aside>
      ...
      <el-form class="profile-form" :model="form" label-position="top">
        ...
        <el-form-item label="班级" required>
          <el-select v-model="form.banjiTypes" clearable filterable :disabled="!form.yuanxiTypes" placeholder="请选择班级">
...
const classOptions = computed(() => allClassOptions.value.filter((option) => filterClassByDepartment(option, form.yuanxiTypes)))
const avatarUrl = computed(() => normalizeAssetUrl(form.xueshengPhoto))
const studentInitial = computed(() => (form.xueshengName || '学').slice(0, 1))

onMounted(async () => {
  await Promise.all([ensure('sex_types'), ensure('yuanxi_types'), ensure('banji_types')])
  await loadProfile()
})
...
function filterClassByDepartment(option, departmentId) {
