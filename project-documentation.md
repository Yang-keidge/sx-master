...
学生实习信息管理系统用于支撑高校学生实习、校企合作招聘、公告通知、讨论交流和师生问题解答等业务。系统面向管理员、学生、老师、企业四类用户，提供从基础档案维护、实习跟踪、招聘应聘到数据统计分析的闭环管理能力。
...
| 角色 | 登录表 | 主要能力 |
| --- | --- | --- |
| 管理员 | `users` | 管理学生、教师、企业、实习、招聘岗位、应聘学生、公告、评论、讨论区、讨论回复、字典、配置，查看全局看板 |
| 学生 | `xuesheng` | 维护个人信息和简历，查看招聘岗位并应聘，查看个人实习，浏览公告并评论，发布讨论帖，向本专业老师提问 |
| 老师 | `laoshi` | 维护所属专业学生，查看学生实习情况，发布公告，查看/维护评论，参与讨论，回复本专业学生问题 |
| 企业 | `qiye` | 维护企业信息，发布招聘岗位，查看应聘学生和简历，录用后生成实习记录，维护本企业实习、公告、评论和讨论 |
...
当前版本已移除就业业务模块：数据库不再创建 `jiuye` 表，后端不再提供就业 CRUD 接口，前端四类角色菜单和首页看板均不再展示就业管理、就业记录、就业学生数、就业率等内容。`db.sql` 保留 `DROP TABLE IF EXISTS \`jiuye\``，用于旧库重新导入时清理历史就业表。公告类型字典仍保留“就业提醒”，用于兼容既有公告数据。
...
`/dashboard/summary` 聚合管理员首页所需的学生、教师、企业、实习、招聘岗位、应聘学生、最新实习记录和最新招聘岗位等数据。
...
`db.sql` 已包含全部表结构和基础字典数据。`db2.sql` 已包含学生、教师、企业、实习、招聘、公告和评论等全部基础数据，并保留原补充演示数据。
...
- 通用统计接口允许动态表名和字段名，公开部署时需要额外限制调用范围。
- 当前仓库仅保留根目录项目说明和完整项目文档；Playwright 验证快照等临时产物已清理。
